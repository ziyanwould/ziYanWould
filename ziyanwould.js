...
var ziYanWould = {
    ...
    //replaceStr('1asd88465asdwqe3',[5],1,'+')        replaceStr('1asd88465asdwqe3',[5],0,'+')
    //result："1asd88465as+++++"                      result："+++++8465asdwqe3"
...
/*// 给用户名写入一个名字
 // document.getElementById('username').value = '老谢';

 // 有id尽量用id去获取元素，因为id选择器速度最快
 $('#username').val('老谢');

 // 给用户名后添加一个冒号
 $('label[for=username]').html('<strong>用户名：</strong>');
 ...
 // 给所有的标题后面加上（*）
 $('label[for]').text(function(idx,oldvalue){
 console.log(arguments);
 return oldvalue + '（*）'
 });

 $('label[for]').css('font-size',function(idx,oldvalue){
 console.log(arguments);
 return parseInt(oldvalue) + idx
 });*/
